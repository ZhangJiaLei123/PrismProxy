...
// （系统浏览器中 window.go/window.runtime 不存在）；数据全部走 fetch（api/）。
// provider 必须挂在 ReviewPage 的祖先——useMessage/useDialog 只能在 provider
...
function Root() {
  return h(
    ...
    { default: () => h(NMessageProvider, null, { default: () => h(NDialogProvider, null, { default: () => h(ReviewPage) }) }) },
  )
}
